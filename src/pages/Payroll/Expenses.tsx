...
import { 
  Wallet, Search, Plus, Pencil, Trash2, X, Save, ChevronLeft, ChevronRight, 
  ...
  Users, Target, ChevronDown, User, Coffee, Car, MonitorSmartphone, Clock
} from 'lucide-react';
...
const INITIAL_EXPENSES = [
  { 
    id: 1, ticketId: 'EXP-2605-001', empId: 'EMP-24001', nameTh: 'สมชาย มุ่งมั่น', nameEn: 'Somchai Mungmun', 
    dept: 'Sales', image: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80',
    date: '2026-05-02', type: 'Travel & Transport', amount: 1250, 
    description: 'ค่าเดินทาง Taxi ไปพบลูกค้า (Makro HQ)', 
    status: 'Pending', hasReceipt: true, rejectReason: ''
  },
  { 
    id: 2, ticketId: 'EXP-2605-002', empId: 'EMP-22045', nameTh: 'วิภาดา แสงงาม', nameEn: 'Wipada Saengngam', 
    dept: 'QA', image: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=150&q=80',
    date: '2026-05-05', type: 'Meals & Entertainment', amount: 3500, 
    description: 'เลี้ยงอาหารกลางวันทีมตรวจสอบคุณภาพหลังผ่าน ISO', 
    status: 'Approved', hasReceipt: true, rejectReason: ''
  },
  { 
    id: 3, ticketId: 'EXP-2605-003', empId: 'EMP-24050', nameTh: 'สมหมาย ใจดี', nameEn: 'Sommai Jaidee', 
    dept: 'Production', image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80',
    date: '2026-05-06', type: 'Office Supplies', amount: 850, 
    description: 'ซื้อถุงมือเซฟตี้เร่งด่วนสำหรับพนักงานใหม่', 
    status: 'Paid', hasReceipt: true, rejectReason: ''
  },
  { 
    id: 4, ticketId: 'EXP-2605-004', empId: 'EMP-24001', nameTh: 'สมชาย มุ่งมั่น', nameEn: 'Somchai Mungmun', 
    dept: 'Sales', image: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80',
    date: '2026-05-08', type: 'Accommodation', amount: 4500, 
    description: 'ค่าโรงแรมจัดบูธงานแสดงสินค้าที่เชียงใหม่', 
    status: 'Rejected', hasReceipt: false, rejectReason: 'ไม่มีใบเสร็จรับเงินฉบับจริง (No original receipt)'
  },
];

const EXPENSE_TYPES = [
    { name: 'Travel & Transport', icon: Car },
    { name: 'Meals & Entertainment', icon: Coffee },
    { name: 'Accommodation', icon: Building2 },
    { name: 'Office Supplies', icon: Briefcase },
    { name: 'IT & Equipment', icon: MonitorSmartphone },
...
const EditExpenseModal = ({ isOpen, onClose, record: activeRecord, onSave }: EditExpenseModalProps) => {
  const [modalStep, setModalStep] = useState(0);
  const [formData, setFormData] = useState<ExpenseRecord>({ 
    id: 0, ticketId: '', empId: 'EMP-24001', nameTh: 'สมชาย มุ่งมั่น', nameEn: 'Somchai Mungmun', 
    dept: 'Sales', image: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80',
    date: '', type: 'Travel & Transport', amount: 0, 
  ...
  useEffect(() => {
    if (activeRecord) {
      ...
      setFormData({ 
        id: 0, ticketId: `EXP-2605-${Math.floor(100 + Math.random() * 900)}`, empId: 'EMP-24001', nameTh: 'สมชาย มุ่งมั่น', nameEn: 'Somchai Mungmun', 
        dept: 'Sales', image: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80',
        date: new Date().toISOString().split('T')[0], type: 'Travel & Transport', amount: 0, 
        description: '', status: 'Pending', hasReceipt: true, rejectReason: ''
      });
    }
    setModalStep(0);
            ...
            <div className="bg-[#212c46] px-5 py-4 flex justify-between items-center text-[#f3f3f1] shrink-0 border-b-2 border-[#b22026]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center border border-white/20">
                  <Wallet size={20} className="text-[#b58c4f]"/>
                </div>
                <h3 className="text-sm font-black uppercase tracking-widest">
                  {activeRecord ? 'Configure Expense Node' : 'Initialize Expense Claim Ticket'}
                </h3>
              </div>
    ...
    >
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden bg-[#f8f9fa] max-h-[75vh]">
          ...
          <div className="w-full md:w-56 bg-white border-b md:border-b-0 md:border-r border-[#eaeaec] flex flex-row md:flex-col shrink-0">
            ...
            {[
              ...
              { id: 1, label: 'Expense Details', icon: Target },
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-white">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      type="text" value={formData.nameEn} onChange={e => {
                        ...
                        let department = formData.dept;
                        if (val.includes('Somchai')) { th = 'สมชาย มุ่งมั่น'; emp='EMP-24001'; img='https://images.unsplash.com/photo-1599566150163-29194dcaad36?auto=format&fit=crop&w=150&q=80'; department='Sales'; }
                        else if (val.includes('Wipada')) { th = 'วิภาดา แสงงาม'; emp='EMP-22045'; img='https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=150&q=80'; department='QA'; }
                        else if (val.includes('Sommai')) { th = 'สมหมาย ใจดี'; emp='EMP-24050'; img='https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80'; department='Production'; }
                        setFormData({...formData, nameEn: val, nameTh: th, empId: emp, image: img, dept: department});
                      }}
            ...
            {modalStep === 1 && (
              <div className="space-y-4 animate-fadeIn pb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-[#606a5f] uppercase tracking-widest">Primary Expense Type (ประเภทการเบิก)</label>
                    <select 
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-[#606a5f] uppercase tracking-widest">Expense Ticket Date (วันที่ใบเสร็จ)</label>
                    <input 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-2.5 bg-[#f8fafc] border border-slate-200 rounded-xl outline-none focus:border-[#b22026] text-[12px] font-bold text-[#212c46]"
                      placeholder="ระบุจำนวนเงินค่าใช้จ่ายเป็นบาท..."
                    />
                  </div>
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-[#606a5f] uppercase tracking-widest">Expense Description Notes (วัตถุประสงค์และรายละเอียดการใช้จ่าย) *</label>
                  <textarea 
                    ...
                    placeholder="ป้อนรายละเอียดและวัตถุประสงค์ทางธุรกิจของการจ่ายเงินตัวนี้ เช่น ค่าเดินทางไป Makro HQ หรือ ซื้อถุงมือเซฟตี้สำหรับพนักงานใหม่ เพื่ออำนวยความสะดวกในการตรวจสอบบัญชี..."
                  />
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 animate-fadeIn pb-6">
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <option value="Approved">Approved (ผ่านการพิจารณาอนุมัติเรียบร้อย)</option>
                    <option value="Paid">Paid (ดำเนินการจ่ายเงินชดเชยสำเร็จ)</option>
                ...
                <div className="p-4 bg-[#3f809e]/5 border border-[#3f809e]/20 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[11px] font-black uppercase text-[#212c46] tracking-wider block">Financial Audit Security Guard</span>
                      <p className="text-[9px] font-medium text-[#7a8b95]">รักษาความลับระดับสูงและบันทึกประวัติการใช้จ่ายขององค์กรอย่างครบถ้วน</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="px-6 py-4 bg-[#f8fafc] border-t border-slate-100 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-1.5 text-[9px] text-[#7a8b95] font-mono font-black uppercase tracking-widest">
            <Info size={13}/> Confidential Record Protected Node under Financial standard
          </div>
...
export default function Expenses() {
  ...
  const [records, setRecords] = useState<ExpenseRecord[]>(INITIAL_EXPENSES);
  ...
  const handleSave = (data: ExpenseRecord) => {
    if (data.id) {
      ...
      setToast('อัปเดตข้อมูลสลักใบเบิกค่าใช้จ่ายสำเร็จเรียบร้อยค่ะ');
    } else {
      const newEntry = { 
        ...data, 
        id: Math.floor(100 + Math.random() * 900), 
        ticketId: `EXP-2605-${Math.floor(100 + Math.random() * 900)}`, 
        date: data.date || new Date().toISOString().split('T')[0]
      };
      setRecords([newEntry, ...records]);
      setToast('อนุมัติสร้างเอกสารเบิกค่าใช้จ่ายสำเร็จเสร็จสิ้นค่ะ');
    }
  ...
  const handleApproveDirect = (id: number) => {
    ...
    setToast('อนุมัติบันทึกเบิกจ่ายวงเงินสำเร็จเรียบร้อยค่ะ');
  ...
  const handleDelete = (id: number) => {
    if (window.confirm('ยืนยันประสงค์ทำลายรายการเบิกค่าใช้จ่ายข้อนี้? ข้อมูลจะไม่ปรากฏในการประเมินเบิกงบประมาณประจำปี')) {
      setRecords(records.filter(r => r.id !== id));
      setToast('ลบชุดพยานเบิกจ่ายออกจากเครื่องมือวิเคราะห์สำเร็จ');
      setTimeout(() => setToast(null), 3000);
    }
  };
  ...
  return (
    <div className="flex flex-1 w-full flex-col animate-fadeIn bg-transparent space-y-4 pb-6">
      ...
      <EditExpenseModal isOpen={modal.isOpen} onClose={() => setModal({isOpen: false, record: null})} record={modal.record} onSave={handleSave} />
      {/* TOAST SYSTEM */}
      {toast && createPortal(
      ...
      <div className="h-14 px-8 flex flex-row items-center justify-between gap-4 z-20 shrink-0 bg-transparent">
          <div className="flex items-center gap-5">
              <div className="relative flex items-center justify-center group cursor-default shrink-0">
                  ...
                  <div className="relative z-10 p-1.5 border border-[#b22026]/40 rounded-xl bg-white/50 backdrop-blur-xs shadow-xs">
                      <Wallet size={28} strokeWidth={2.5} className="text-[#b22026]" />
                  </div>
              </div>
              <div>
                  <h3 className="font-black text-[#212c46] uppercase tracking-tighter leading-none" style={{ fontSize: '24px' }}>
                      EXPENSE <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#b22026] to-[#b58c4f]">CLAIMS</span>
                  </h3>
                  <p className="text-[11px] font-bold text-[#4d5a44] uppercase tracking-[0.2em] mt-0.5 opacity-80 leading-none">
                      REIMBURSEMENT & CORPORATE EXPENSES CONTROL NODE
                  </p>
              </div>
          </div>
          ...
          <div className="flex items-center gap-4">
              ...
              >
                <Plus size={16} /> Register Expense Ticket
              </button>
          </div>
      </div>
      <div className="px-4 sm:px-8 w-full mt-[2px]">
        <div className="w-full animate-fadeIn pb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-3 shrink-0">
                <KpiCard label="Total Expense Claims" value={records.length} icon={Receipt} color={THEME.palette.brick} description="Claims Count" />
            ...
            {/* MAIN DATA BLOCK */}
            <div className="bg-white/90 rounded-3xl shadow-lg border border-[#eaeaec] overflow-hidden flex flex-col transition-all  flex-1 min-h-0">
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto">
                        <div className="relative flex-1 md:w-80">
                            ...
                            <input 
                              ...
                              placeholder="Search employee, Ticket ID, expense type..." 
                    ...
                    <table className="w-full text-left border-collapse min-w-[1000px]">
                        <thead className="bg-[#222b38] text-white">
                            <tr className="border-b-2 border-[#709654]">
                                ...
                                <th className="py-4 px-4 font-black uppercase tracking-widest text-[12px] text-center w-1/3">Expense Details & Descriptions</th>
                        ...
                        <tbody className="bg-white divide-y divide-[#eaeaec]">
                            {filteredRecords.map(r => {
                                const typeInfo = EXPENSE_TYPES.find(t => t.name === r.type) || { icon: Receipt };
                                const TypeIcon = typeInfo.icon;
                                
                                return (
                                <tr key={r.id} className="hover:bg-[#f8f9fa] transition-colors group">
                                    <td className="py-2.5 px-4">
                                        <p className="font-mono font-black text-[#7a8b95] text-[12px]">{r.ticketId}</p>
                                    ...
                                    <td className="py-2.5 px-4">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div>
                                                <span className="font-black text-[#212c46] text-[12px] block">{r.type}</span>
                                                <p className="text-[10px] font-bold text-[#7a8b95] leading-relaxed mt-0.5 max-w-sm line-clamp-1">{r.description}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-2.5 px-4 text-right">
                                        <p className="font-mono font-black text-[#212c46] text-[12px]">฿{formatCurrency(r.amount)}</p>
                                        <div className="mt-0.5">
                                            {r.hasReceipt ? (
                                                <span className="inline-flex items-center gap-1 text-[9px] font-black text-[#508660] uppercase tracking-wider font-mono">
                                                    <Receipt size={10}/> Attached
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1 text-[9px] font-black text-[#b22026] uppercase tracking-wider font-mono">
                                    ...
                                    <td className="py-2.5 px-4 text-center">
                                        <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-black uppercase tracking-widest font-mono border
                                    ...
                                    <td className="py-2.5 px-4 text-center">
                                        <div className="flex justify-center items-center gap-[1px]">
                                            {r.status === 'Pending' && (
                                                <>
                                                    <button onClick={() => handleApproveDirect(r.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#508660] hover:bg-[#508660]/10 active:scale-95 transition-all cursor-pointer" title="Approval Node">
                                                        <Check size={14} strokeWidth={3} />
                                                    </button>
                                                    <button onClick={() => handleRejectDirect(r.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#b22026] hover:bg-[#b22026]/10 active:scale-95 transition-all cursor-pointer" title="Reject Node">
                                                        <XCircle size={14} />
                                                    </button>
                                                </>
                                            )}
                                            {r.status === 'Approved' && (
                                                <button onClick={() => setRecords(records.map(itm => itm.id === r.id ? { ...itm, status: 'Paid' } : itm))} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#3f809e] hover:bg-[#3f809e]/10 active:scale-95 transition-all cursor-pointer" title="Mark as Paid">
                                                    <Banknote size={14} />
                                                </button>
                                            )}
                                            <button onClick={() => setModal({isOpen: true, record: r})} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#606a5f] hover:bg-[#f3f3f1] active:scale-95 transition-all cursor-pointer" title="Config Node">
                                                <Pencil size={12} />
                                            </button>
                                            <button onClick={() => handleDelete(r.id)} className="w-8 h-8 flex items-center justify-center rounded-lg text-[#7a8b95] hover:bg-red-50 hover:text-[#b22026] active:scale-95 transition-all cursor-pointer" title="Purge Node">
                                                <Trash2 size={12} />
                                            </button>
                                        </div>
                                    </td>
                            ...
                            {filteredRecords.length === 0 && (
                                <tr>
                                    <td colSpan={6} className="py-12 text-center text-[12px] font-black uppercase text-[#7a8b95] tracking-widest bg-white">
                                        No expense records detected inside active filter parameters
                                    </td>
                                </tr>
                            )}
                        </tbody>
            ...
            {/* MARGIN SPACING OF 32px (mt-8) SEPARATING THE MAIN END NODE FROM GENERAL APP FOOTER */}
