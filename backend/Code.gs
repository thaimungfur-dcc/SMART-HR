...
const GLOBAL_SHEETS_CONFIG = {
    ...
    'Attendance': ['id', 'employeeId', 'employeeName', 'date', 'checkIn', 'checkOut', 'status', 'hours', 'shift', 'mode', 'remarks'],
    'RawScannerLogs': ['id', 'acNo', 'name', 'dept', 'date', 'morningIn', 'morningOut', 'afternoonIn', 'afternoonOut', 'otIn', 'otOut', 'checkIn', 'checkOut', 'hours', 'status', 'remarks', 'matchedEmployeeId', 'matchedEmployeeName', 'isMatched', 'isProcessed', 'processedAt'],
  ...
  try {
    ...
    if (!sheet && sheetName) {
      ...
      if (columns && columns.length > 0) {
        if (columns.length > sheet.getMaxColumns()) {
...
function writeData(sheet, data, headersObj) {
  ...
  const requiredRows = lastRow + rows.length;
